...
- **Key sub-signals** (see Tier 4 for detail):
  ...
  - Fiscal understatement: council-authored items claiming "None" or "Staff time" fiscal impact when the scope clearly incurs real obligations — new permit regimes, bans, citywide policy frameworks. Detected via `check_fiscal_understatement()` in `agenda_scraper.py`; scored −0.015/item authored, −0.007/item cosponsored, capped at −0.04. The failure may reflect naivete (not understanding what the proposal will require), weak effort (not thinking it through), or disregard for transparency — the penalty applies regardless of motive because the result is the same: the council and public lack a realistic cost picture when the item is heard.
