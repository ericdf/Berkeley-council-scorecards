...
- **Financial Condition Audit (April 2026):** The City Auditor's FY2016–FY2025 financial condition review documents a city in structural imbalance, with no corrective trajectory visible in council action:
  ...
  - **Staffing growth:** FTE count grew 30%, from 1,336 to 1,747. The audit does not assess productivity per employee; no council member has publicly requested this analysis.
...
- **Tregub fiscal referral data gap:** Tregub is widely known as the architect of the bond schedule optimization referral, but the agenda JSON parser does not currently find his name as primary author of that item. As a result `fiscal_referral_authored = 0` for Tregub, which underestimates his revenue-seeking activity via the referral channel. A manual correction or improved author-matching pass is needed.
